<template>
  <div class="box">
    ...
    <div style="width:80%;paddingBottom:5px;float:left;paddingLeft:50px;">
      ...
      <el-button type="primary" size="mini" style="float: left" @click="Delwd(2)">下载明细2</el-button>
    ...
    <div class="table" style="display: none">
      <el-table size="mini" :data="TableData" highlight-current-row :height="this.$store.state.TableOptimumHeight" border style="width: 100%" id="out-table" :header-cell-style="{ background: '#e1e1e1' }">
        ...
        <el-table-column prop="MaintainCode" width="100" label="订单单号"></el-table-column>
    ...
    <div class="table" style="display: none">
      <el-table size="mini" :data="TableData" highlight-current-row :height="this.$store.state.TableOptimumHeight" border style="width: 100%" id="out-table2" :header-cell-style="{ background: '#e1e1e1' }">
        <el-table-column prop="CusFullName" width="100" label="客户名称"></el-table-column>
        <el-table-column prop="CusType" width="100" label="客户类型"></el-table-column>
        <el-table-column prop="Area" width="100" label="区域"></el-table-column>
        <el-table-column prop="PayDate" width="100" label="付款日期"></el-table-column>
        <el-table-column prop="MaintainCode" width="100" label="合同编码"></el-table-column>
        <el-table-column prop="ConrtractType" width="100" label="合同类型"></el-table-column>
        <el-table-column prop="Amount" width="100" label="合同金额"></el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 下载明细
    Delwd: function (index) {
      var urlMCSalesDetail = "Statistic/MCSalesDetail";
      this.nameExcel = "#out-table";

      if (index == "2") {
        urlMCSalesDetail = "Statistic/MCSalesDetail2";
        this.nameExcel = "#out-table2";
      }
      var obj = {
        api: "get",
        url: this.$store.state.http + urlMCSalesDetail,
        data: {
          begin: this.begin, //开始时间
          end: this.end, //结束时间
          cusType: this.CustomerType, //客户类型 宝马、保时捷.....
    ...
    exportExcel() {
      ...
      var wb = XLSX.utils.table_to_book(
        document.querySelector(this.nameExcel),
        xlsxParam
      );
      console.log(wb);
      /* 获取二进制字符串作为输出 */
