<template>
  <div class='box'>
    ...
    <el-table :data="Data" size="mini" :height="this.$store.state.TableOptimumHeight" :row-class-name="tableRowClassName" highlight-current-row border style="width: 100%" :header-cell-style="{ background: '#e1e1e1' }">
      ...
      <el-table-column prop="EmployeeName" label="工程师" sortable></el-table-column>
      <el-table-column prop="Pone" label="初核分数"></el-table-column>
      <el-table-column prop="Cone" label="复合分数"></el-table-column>
      <el-table-column prop="IsManQinStr" label="满勤"></el-table-column>
      <el-table-column prop="JaJa" label="加减"></el-table-column>
    ...
    <div class="content" style="display: none">
      <el-table :data="DataStr" size="mini" id="#out-table" :height="this.$store.state.TableOptimumHeight" :row-class-name="tableRowClassName" highlight-current-row border style="width: 100%"
        ...
        <el-table-column prop="Date" label="月份" width="90" sortable></el-table-column>
        <el-table-column prop="Quyu" label="区域" width="100" sortable></el-table-column>
        <el-table-column prop="EmployeeName" label="工程师" width="90" sortable></el-table-column>
        <el-table-column prop="Pone" label="初核分数" width="120"></el-table-column>
        <el-table-column prop="Cone" label="复合分数" width="120"></el-table-column>
        <el-table-column prop="IsManQinStr" label="满勤" width="120"></el-table-column>
        <el-table-column prop="JaJa" label="加减"></el-table-column>
    ...
    <!-- <div class="pagetion">
      <el-pagination class="pagition-li" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="1" :page-sizes="this.$store.state.Vuex.pageSNumber" :page-size="GetKPIsEsach.pagesize"
...
export default {
  ...
  methods: {
    ...
    init: function () {
      ...
      var obj = {
        api: "get",
        url: this.$store.state.http + "PCenter/GetKPIs",
      ...
      this.$https(obj).then((res) => {
        ...
        for (let index = 0; index < this.Data.length; index++) {
          ...
          element.IsManQin == 0
            ? (element["IsManQinStr"] = "否")
            : (element["IsManQinStr"] = "是");
          element.Status == 1
            ? (element["StatusStr"] = "已初核")
            : (element["StatusStr"] = "已复核");

          element["JaJa"] = "";
          if (element.SpecialJia != null && element.SpecialJia != "") {
            element.JaJa +=
              "[" + element.SpecialJiaM + " 加 " + element.SpecialJia + "]";
          }
          if (element.SpecialJian != null && element.SpecialJia != "") {
            element.JaJa +=
              "[" + element.SpecialJianM + " 减 " + element.SpecialJian + "]";
          }
        }
      });
    },
    ...
    Excel: function () {
      ...
      this.$https(obj).then((res) => {
        ...
        for (let index = 0; index < this.DataStr.length; index++) {
          const element = this.DataStr[index];
          element.IsManQin == 0
            ? (element["IsManQinStr"] = "否")
            : (element["IsManQinStr"] = "是");
          element.Status == 1
