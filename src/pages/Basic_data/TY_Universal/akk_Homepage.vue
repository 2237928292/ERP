  ...
  <div class="box_s">
    ...
    <div class="demo">
      ...
      <el-row class="demo-autocomplete">
        <el-col>
          <el-autocomplete class="inline-input input" v-model="Product.Manufacturer" @select="handleSelect" :fetch-suggestions="querySearch" @input="BuyingUnitPriceOnclick" placeholder="请输入内容"></el-autocomplete>
        </el-col>
      </el-row>
    </div>
    ...
    <div class="demo">
      <div>年份:</div>
      <el-input v-model="Product.ProductionYear" size="mini"></el-input>
    </div>
    <!-- 保质期 -->
    <div class="demo">
      <div>保质期:</div>
    ...
    <div class="true">
      <el-button type="primary" style="float:right" size="mini" @click="add()">保存</el-button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    Manufacturer: function () {
      ...
      this.$https(obj).then((res) => {
        ...
        // this.Product.ProductionYear = String(this.Product.ProductionYear)
        this.restaurants = restaurants;
        console.log(restaurants);
        (this.$store.state.HomepageData.BMBSJ = "35"),
          (this.Product = this.$store.state.HomepageData);
        if (this.Product.AssemblyUnitPrice == "")
          this.Product.AssemblyUnitPrice = 0;
      });
    },
    // 品牌输入框
    ...
    add: function () {
      ...
      var obj = {
        api: "post",
        url: this.$store.state.http + "Product/UpdateProduct",
        data: {
          ID: this.Product.ID,
      ...
      this.$https(obj).then((res) => {
        if (res.Code == 0) {
          this.$emit("refresh"), this.$router.push({ path: "/TY_Universal" });
        }
      });
    },
  },
